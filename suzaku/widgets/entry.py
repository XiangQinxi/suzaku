...
class SkEntry(SkVisual):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__(
            *args, size=size, style=style, name="sk_entry", id=id, **kwargs
        )
    ...
    def _key(self, evt: Event):
        ...
        if evt.key == KEY_BACKSPACE:
            if cursor_pos > 0:
                ...
                if self.visual_attr["textvariable"] is not None:
                    self.visual_attr["textvariable"].set(
        ...
        if not self.is_focus:
            if self.visual_attr["placeholder"] and not text:
                ...
                canvas.drawSimpleText(
                    self.visual_attr["placeholder"], draw_x, draw_y, font, text_paint
                )
            else:
                # 绘制文本（考虑滚动和裁剪）
                canvas.drawSimpleText(
            ...
            if cursor_x - scroll_offset > visible_width:
                # 如果光标超出可见区域右侧，调整滚动偏移
                self.visual_attr["scroll_offset"] = (
                    cursor_x - visible_width + 5
                )  # 加5像素的边距
                scroll_offset = self.visual_attr["scroll_offset"]
            elif cursor_x - scroll_offset < 0:
                # 如果光标超出可见区域左侧，调整滚动偏移
                self.visual_attr["scroll_offset"] = max(
                    0, cursor_x - 5
                )  # 加5像素的边距
                scroll_offset = self.visual_attr["scroll_offset"]
            ...
            if (
                current_time - self.visual_attr["last_blink_time"]
                >= self.visual_attr["blink_interval"]
            ):
                # 切换光标可见性
                self.visual_attr["cursor_visible"] = not self.visual_attr[
                    "cursor_visible"
                ]
